...
add_module(Module, Args, ClientState) ->
								 case gen_client_utils:has_behaviour(Module, gen_client) of 
									 true ->
										 case Module:init(self(), Args) of
											 {ok, ModuleRef} ->
												 add_handler_internal({fun(Received, State) -> Module:handle(ModuleRef, Received, State) end,
																							 fun(State) -> Module:terminate(ModuleRef, State) end}, ClientState);
											 ok ->
...
add_handler(Client, Handler) when is_function(Handler) ->
